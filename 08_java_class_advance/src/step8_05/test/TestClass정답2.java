...
//5:00~6:00  6:00~8:00		
		...
		for (int i = 0; i < mg.memberList.size(); i++) {
			...
			String name1= total2[i];
			
			int idx=i;
			for (int j = i; j < mg.memberList.size(); j++) {
				
				if(name1.compareTo(total2[j])<0) {
					name1=total2[j];
					idx=j;
				}
			}
			String temp=total2[i];
			total2[i]=total2[idx];
			total2[idx]=temp;	
		}
		...
		for (int i = 0; i < mg.memberList.size(); i++) {
			...
			for (int j = 0; j < mg.memberList.size(); j++) {
					if(total2[i].equals(Integer.toString(total[j]))) {
						cnt++;
						idx=j;
					}
			...
			if(cnt==6) {
				idx--;
				userdata+=mg.memberList.get(idx).getCustno();
				userdata+="\t";
				userdata+=mg.memberList.get(idx).getCustname();
				userdata+="\t";
				userdata+=total2[i];
				userdata+="\n------------------\n";
			}
