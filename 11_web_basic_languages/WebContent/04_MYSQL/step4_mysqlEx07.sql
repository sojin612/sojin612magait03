/*
    ...
    # 숫자 관련함수 
		...
		3) FLOOR	: 소수점을 버린수를 가져온다.(소수점 밑을 버림)
		4) ROUND	: 소수점을 반올림한 값을 가져온다. (자릿수를 지정가능하다.)
		5) TRUNCATE : 소수점을 버린 값을 가져온다. 	   (자릿수를 지정해서 버림.)
		6) POW		: x의 y승을 가져온다.
		7) MOD		: 나머지값을 가져온다. MOD(10,3)가 1?? 10을 3으로 나눈값의 나머지 1을 반환한다.
		8) GREATEST : 최대값을 가져온다.
		9) LEAST    : 최소값을 가져온다.
		FLOOR 과 TRUNCATE 의 차이점?? FLOOR은 소수점밑을버림,TRUNCATE는 자릿수지정해서버림.
