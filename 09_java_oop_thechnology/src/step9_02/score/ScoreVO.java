...
public class ScoreVO { //vo==value object //데이터묶음.
	...
	public ScoreVO() { }  //왜 생성자를 두번 선언하나??기본생성자를 만든것임.
	public ScoreVO(String name, int java, int jsp, int spring) {
		...
		this.jsp = jsp;
		this.spring = spring;
		total = java + jsp + spring;   //생성자가 생성될때 같이 처리됨. 생성자는 딱한번만 생성되는데 계속생성되도 되나??ㅇㅇ 생성자는 get set이 필요없나?? getter setter는 private에서 가져와서 변수에 담을때만 외부에서 값을 가져와서 생성하는 생성자는 필요없음.
		this.average = (double) total / 3;
	}
	public int getNo() {
		return no;
	}
	public void setNo(int no) { //어디서 no를 입력받나?? 여기서는 getter setter는 안쓰임. 만약에 setter값을 수정하고 싶으면 scoreList 클래스에서 수정해줌.
		this.no = no;
	}
	...
	@Override
	public String toString() { // 어디로 리턴?? scoreList클래스의 향상된for문의 vo로 
		return String.format("   %d   %s %03d  %03d    %03d    %03d  %6.2f %d", no, name, java, jsp, spring, total, average, rank);
	}
