...
public class ArrayEx44_문제 {
	...
	public static void main(String[] args) {
		...
		for (int i=0; i<temp.length; i++) {
			...
			for (int j=0; j<i; j++) { //j<i 인 이유 [i]=7 이고 i가 0이면 7이 처음나온것이고중복이 아님으로 [0]에 저장됨. [i]=10이고 i=1이면 j가 10이 i의 1전에 나온적이 있는지,즉 0을 중복검사하니깐 j<i이어야 함 
				if (temp[i] == temp[j]) {
					i -= 1;
				}
			}
		...
		while (true) {
			...
			// 가로검사
			for (int i=0; i<maxNum; i++) {
				int cnt = 0;     //for안에 없으면 cnt가 연속이아니어도 ==5가 됨
				for (int j=0; j<maxNum; j++) {
					if (mark[i][j] == 1) {
						cnt += 1;
					}
				}
				if (cnt == 5) {  //for밖에 있으면 가로줄 모두가 돔
					win = 1;
					break;
				}
			}
