	...
	public static void main(String[] args) {
		...
		while(true) {
			...
			if (sel == 1) {
				...
				if (elementCnt-1 < delIdx || delIdx < 0) { //elementCnt-1 을 하는 이유- 0이 있기 때문
				...
				else if(elementCnt > 1) {     //성적이 100, 99, 88
					int[] temp = score;
					score = new int[elementCnt - 1]; 
					
					for (int i=0; i<delIdx; i++) {  //100,99
						score[i] = temp[i];
					}
					for (int i=delIdx; i<elementCnt-1; i++) {  //99,88
						score[i] = temp[i + 1];
					}
					temp = null;
				}
				...
				if (elementCnt == 1) {
					...
					for(int i=0; i<elementCnt; i++) {
						if(i != delIdx) {
							score[j] = temp[i]; //temp에 delidx를 제외한 것을 옮겨담아야함으로
							j += 1;
						}
					}
				...
				if (elementCnt == 0) {
					...
					for(int i=0; i<elementCnt + 1; i++) {
						if(i != insertIdx) {
							score[i] = temp[j];//temp에 있는것을 빠짐없이 옮겨담아야함으로 10,20,30
							j += 1;
						}
					}
					temp = null;
